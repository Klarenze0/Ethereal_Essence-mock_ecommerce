...
if (isset($_GET['search'])) {
    $searchInput = trim($_GET['search']); // Get the input and trim extra spaces

    if (!empty($searchInput)) {
      

        // Proceed with your search logic, e.g., querying a database
        // Assuming you have a database connection stored in $conn
        
        $searchInput = "%" . $searchInput . "%"; // Add wildcard characters for partial match
        
        // SQL query to search for the term in product_name or other columns in the table
        $search_sql = "SELECT * FROM product_info WHERE product_name LIKE ?";
        
        if ($stmt = $conn->prepare($search_sql)) {
            // Bind the search input to the prepared statement
            ...
            echo "Error preparing the search query.";
        }
    } else {
  ...
  <div class="search">
    ...
    <button type="submit" class="btn">Search</button>
      ...
      <div class="topfavorite">
        <div class="container">
        <div class="favorite">
    You searched for: <?php echo htmlspecialchars(substr($searchInput, 1, -1)); ?>
</div>
      ...
      <div class="mensperfume" style = "height: 750px;">
...
if ($stmt = $conn->prepare($search_sql)) {
    ...
    // Check if any results were returned
    if ($result->num_rows > 0) {
        // Loop through the products and display them inside your HTML structure
        while ($row = $result->fetch_assoc()) {
            echo '
            <div class="product-item">
                <a href="product.php?product_id=' . $row['product_id'] . '">
                    <img src="' . $row['image'] . '" alt="' . htmlspecialchars($row['product_name']) . '" />
                    <p class="perfname">' . htmlspecialchars($row['product_name']) . '</p>
                    <p class="price">P ' . htmlspecialchars($row['price']) . '</p>
                </a>
