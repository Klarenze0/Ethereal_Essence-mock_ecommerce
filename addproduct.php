...
if (isset($_POST['save'])) {
    ...
    $producting = $_POST['producting']; // Notice the corrected variable name

    // Prepare the SQL query to insert into the product_info table
    $sql = "INSERT INTO product_info (product_name, price, description, category, stocks, image, ingredients)
            VALUES (?, ?, ?, ?, ?, ?, ?)";

    // Prepare statement
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        // Bind parameters to the query (type 's' for string, 'i' for integer)
        $stmt->bind_param("ssssiss", $productName, $productPrice, $productDescription, $productCategory, $productStock, $path, $producting);
        ...
        if ($stmt->execute()) {
            ...
            echo "Error inserting product information: " . $stmt->error;
        }
    } else {
...
<body>
    ...
    <div class="form-group">
        ...
        <input type="number" id="productPrice" name="productPrice" step="1" value="" required>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <input type="number" id="productStock" name="productStock" value="" required>
    </div>

    <div class="form-group">
        <label for="producting">Ingridients:</label>
        <input type="text" id="producting" name="producting" value="" required>
    </div>
